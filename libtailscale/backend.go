...
func (a *App) runBackend(ctx context.Context, hardwareAttestation bool) error {
	// Install the Android network selector before constructing the backend. The
	// login/control client can create sockets before VpnService is active, while
	// a rescue session may already have selected cellular for the control plane.
	a.setAndroidBindToNetworkFunc()
	...
	for {
		...
		case s := <-onVPNRequested:
			...
			a.setAndroidBindToNetworkFunc()
			b.netstack.SetForwardSocketBinder(func(fd int, destination netip.AddrPort) error {
				if !s.BindSocketToRescueNetwork(int32(fd), destination.String()) {
					return fmt.Errorf("Android rescue network binding failed for %s", destination)
				}
				return nil
			})
		...
		case s := <-onDisconnect:
			if vpnService.service != nil && vpnService.service.ID() == s.ID() {
				...
				// Keep control-plane sockets destination-bound after VPN teardown;
				// the protect hook is the part that requires an active VpnService.
				a.setAndroidBindToNetworkFunc()
				b.netstack.SetForwardSocketBinder(nil)
...
// setAndroidBindToNetworkFunc installs the destination-independent hook used by
// Tailscale's control/DERP sockets. It is intentionally safe to install before
// VpnService exists; the protect hook is added separately when the VPN starts.
func (a *App) setAndroidBindToNetworkFunc() {
	netns.SetAndroidBindToNetworkFunc(func(fd int) error {
		if ok := a.appCtx.BindSocketToNetwork(int32(fd)); !ok {
			if a.appCtx.IsRescueNetworkMode() {
				return fmt.Errorf("cellular control-plane network unavailable for socket %d", fd)
			}

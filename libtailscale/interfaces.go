...
type AppContext interface {
	...
	// IsRescueNetworkMode reports whether destination-aware rescue routing is
	// active. The Android network hook uses this to turn a missing cellular
	// control-plane Network into a hard failure instead of a silent fallback.
	IsRescueNetworkMode() bool

	// BindSocketToRescueNetwork protects and binds a subnet-forwarding socket
	// according to the destination-aware Android rescue-network policy.
	BindSocketToRescueNetwork(fd int32, destination string) bool
}
...
type IPNService interface {
	...
	// BindSocketToRescueNetwork protects and binds an outbound subnet-forwarding
	// socket according to the destination-aware Android rescue-network policy.
	// It must return false when the selected network is unavailable so callers
	// can fail closed instead of silently falling back to another interface.
	BindSocketToRescueNetwork(fd int32, destination string) bool

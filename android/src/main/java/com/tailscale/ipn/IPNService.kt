...
open class IPNService : VpnService(), libtailscale.IPNService {
  ...
  override fun updateVpnStatus(status: Boolean) {
    // VPN state is published through Notifier; this callback remains part of the bridge API.
  }

  override fun bindSocketToRescueNetwork(fd: Int, destination: String): Boolean {
    if (!protect(fd)) {
      TSLog.w(TAG, "保护救援转发 socket 失败: fd=$fd destination=$destination")
      return false
    }
    return app.bindSocketToRescueNetwork(fd, destination)
  }

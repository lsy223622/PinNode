...
object NetworkChangeCallback {
  ...
  fun refreshSelectedNetwork(dns: DnsConfig) {
    lock.withLock { maybeUpdateDNSConfig("network policy changed", dns) }
  }
  ...
  private fun pickDefaultNetwork(): Network? {
    ...
    // Prefer a network that is not metered (typically Wi-Fi); otherwise use
    // the first eligible network.
  ...
  private fun pickCellularNetworkLocked(): Network? {
    return activeNetworks
        .asSequence()
        .filter { (_, info) ->
          info.caps.hasTransport(NetworkCapabilities.TRANSPORT_CELLULAR) &&
              info.caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_INTERNET) &&
              info.caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_NOT_VPN)
        }
        .sortedByDescending { (_, info) ->
          info.caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_VALIDATED)
        }
  ...
  private fun maybeUpdateDNSConfig(why: String, dns: DnsConfig) {
    val defaultNetwork =
        if (forceCellularForControl) pickCellularNetworkLocked() else cachedDefaultNetwork
    ...
    val info =
        if (forceCellularForControl) activeNetworks[defaultNetwork] else cachedDefaultNetworkInfo
    if (info == null) {

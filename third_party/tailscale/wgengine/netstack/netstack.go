...
type Impl struct {
	...
	// forwardSocketBinder, when non-nil, binds sockets created for subnet
	// forwarding to a platform-selected network. The destination is passed to
	// the binder so callers can choose a network per route instead of applying
	// a process-wide binding.
	forwardSocketBinder atomic.Pointer[forwardSocketBinderHolder]
...
// ForwardSocketBinder binds an already-created outbound socket to the network
// that should be used for the destination. The callback must perform any
// platform-specific VPN protection before binding the socket.
//
// It is called for TCP sockets before connect and for UDP sockets immediately
// after listen. Returning an error fails closed and prevents forwarding.
type ForwardSocketBinder func(fd int, destination netip.AddrPort) error

type forwardSocketBinderHolder struct {
	fn ForwardSocketBinder
}

// SetForwardSocketBinder configures the destination-aware binder used by
// subnet TCP and UDP forwarding. Passing nil restores the standard unbound
// net.Dialer/net.ListenUDP behavior.
func (ns *Impl) SetForwardSocketBinder(fn ForwardSocketBinder) {
	if fn == nil {
		ns.forwardSocketBinder.Store(nil)
		return
	}
	ns.forwardSocketBinder.Store(&forwardSocketBinderHolder{fn: fn})
}
...
func (ns *Impl) forwardTCP(getClient func(...tcpip.SettableSocketOption) *gonet.TCPConn, clientRemoteIP netip.Addr, wq *waiter.Queue, dialAddr netip.AddrPort, isLocal bool) (handled bool) {
	...
	} else if holder := ns.forwardSocketBinder.Load(); holder != nil && !isLocal && !dialAddr.Addr().IsLoopback() {
		binder := holder.fn
		dialFunc = func(ctx context.Context, network, address string) (net.Conn, error) {
			destination, err := netip.ParseAddrPort(address)
			if err != nil {
				return nil, fmt.Errorf("netstack: parse forwarding destination %q: %w", address, err)
			}
			var d net.Dialer
			d.Control = func(_ string, _ string, rawConn syscall.RawConn) error {
				var bindErr error
				if err := rawConn.Control(func(fd uintptr) {
					bindErr = binder(int(fd), destination)
				}); err != nil {
					return err
				}
				return bindErr
...
func (ns *Impl) forwardUDP(client *gonet.UDPConn, clientAddr, dstAddr netip.AddrPort) {
	...
	if holder := ns.forwardSocketBinder.Load(); holder != nil && !isLocal && !isLoopback {
		var bindErr error
		rawConn, err := backendConn.SyscallConn()
		if err == nil {
			err = rawConn.Control(func(fd uintptr) {
				bindErr = holder.fn(int(fd), dstAddr)
			})
		}
		if err != nil || bindErr != nil {
			backendConn.Close()
			ns.logf("netstack: refusing UDP forwarding to %v because network binding failed: %v", dstAddr, errors.Join(err, bindErr))
			return
		}
	}
